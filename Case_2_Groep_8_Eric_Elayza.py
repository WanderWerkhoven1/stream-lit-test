...
if option == 'Intro':
    ...
    # Bronvermelding
    st.write(
        "De gegevens zijn verstrekt door CBS Open Data StatLine en zijn gebaseerd op rapporten van 2011 tot en met 2022. Daarnaast hebben we geografische data gebruikt van de websites gadm.org en ec.europa.eu om landkaarten van Nederland te genereren."
    )
    ...
    with tabs[1]:
        # Streamlit-app voor gdf_nederland
        st.write('Je bevindt je op het Inkomen overzicht.')


        # Streamlit-app voor gdf_nederland
        st.header('Het gestandaardiseerd inkomen in Nederland')
        st.write('Ontdek de gegevens over het gemiddeld gestandaardiseerd inkomen in Nederland door de jaren heen.')
    ...
    with tabs[2]:
        st.write('Je bevindt je bij de Scatterplot')
        # Streamlit-applicatie
        st.header('Scatterplot met Trendlijn: Diefstallen vs. Inkomen per Jaar')
        st.write('Ontdek de relatie tussen het aantal diefstallen en het gemiddelde inkomen in deze scatterplot')
